...
part 'delete_body_info_event.dart';
part 'delete_body_info_state.dart';

class DeleteBodyInfoBloc
    extends Bloc<DeleteBodyInfoEvent, DeleteBodyInfoState> {
  final APIBodyInfo apiBodyInfo;
  DeleteBodyInfoBloc(this.apiBodyInfo) : super(StateDeleteBodyInfoInitial()) {
    on<EventDeleteBodyInfoInitial>((event, emit) {
      emit(StateDeleteBodyInfoInitial());
    });
    on<EventDeleteBodyInfoFetching>((event, emit) async {
            ...
            callback: () async {
              try {
                ...
                if (res.data.isNotEmpty) {
                  ...
                  event.context.pop();
                  var data = DeleteBodyInfoRequestModel(
                    id: id,
                    nickname: nickname,
                    address: address,
                    phone: phone,
                  ...
                  final response = await apiBodyInfo.delete(data: data);
                  ...
                  // ignore: use_build_context_synchronously
                  event.context.pop();
                  const message = 'ไม่พบข้อมูลที่ต้องการลบ';
                  // ignore: use_build_context_synchronously
                  dialog.show(
                    context: event.context,
                    accept: 'ตกลง',
